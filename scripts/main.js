...
if (document.location.pathname.includes("custom.html")) {
    function clearInputs() {
        ...
        document.getElementById('c-gender').value = 'Female';
    }

    function createContestantHandler() {
        const nameInput = document.getElementById('c-name');
        const imageInput = document.getElementById('c-image');
        const genderInput = document.getElementById('c-gender');

        const name = nameInput.value.trim();
        const image = imageInput.value.trim() || 'https://placehold.co/100x100/800080/ffffff?text=' + name.substring(0, 2).toUpperCase();
        const gender = genderInput.value;

        if (!name) {
            alert("Please enter a name for the contestant.");
            return;
        }

        const newGuest = new Contestant(name, image, gender);
        const guests = loadCustomGuests();

        if (guests.some(g => g._name.toLowerCase() === name.toLowerCase())) {
            alert(`Contestant with name '${name}' already exists.`);
            return;
        }

        guests.push({
            _name: newGuest.name,
            _image: newGuest.image,
            _gender: newGuest.gender
        });

        saveCustomGuests(guests);
        clearInputs();
        renderCustomGuests();
        alert(`Contestant ${name} created successfully!`);
    }

    function renderCustomGuests() {
        const listContainer = document.querySelector('#castmates-list');
        ...
        if (guests.length === 0) {
            listContainer.innerHTML = '<p style="opacity: 0.7;">No custom guests created yet.</p>';
            return;
        }
        ...
        guests.forEach((guest, index) => {
            ...
            <img 
                src="${guest._image}" 
                alt="${guest._name}"
                onerror="this.onerror=null; this.src='https://placehold.co/100x100/1abc9c/ffffff?text=X';">
            <p>${guest._name}</p>
            <button class="remove-custom-guest" data-index="${index}">Remove</button>
        `;
    ...
    function removeCustomGuestHandler(index) {
        let guests = loadCustomGuests();
    ...
    document.addEventListener('DOMContentLoaded', () => {
        if (document.querySelector('#castmates-list')) {
            renderCustomGuests();

            const createButton = document.getElementById('create');
            if (createButton) {
                createButton.addEventListener('click', createContestantHandler);
            }

            const clearButton = document.getElementById('clear-custom-guests');
            if (clearButton) {
                clearButton.addEventListener('click', clearCustomGuestsHandler);
            }

            const listContainer = document.querySelector('#castmates-list');
            listContainer.addEventListener('click', (event) => {
                if (event.target.classList.contains('remove-custom-guest')) {
                    const index = parseInt(event.target.dataset.index);
                    removeCustomGuestHandler(index);
                }
            });
        }
    });
